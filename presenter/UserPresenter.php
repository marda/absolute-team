...
class UserPresenter extends TeamBasePresenter
{
    ...
    {
        $ret = $this->userManager->getTeamList($resourceId);
        if (!$ret)
    ...
    {
        $ret = $this->userManager->getTeamItem($resourceId, $subResourceId);
        if (!$ret)
            $this->httpResponse->setCode(Response::S404_NOT_FOUND);
        else
        {
    ...
    private function _postUserRequest($urlId, $urlId2)
    {
        ...
        $ret = $this->userManager->userTeamCreate($urlId, $urlId2);
        if (!$ret)
    ...
    private function _deleteUserRequest($urlId, $urlId2)
    {
        ...
        $ret = $this->userManager->userTeamDelete($urlId, $urlId2);
        if (!$ret)
            $this->httpResponse->setCode(Response::S404_NOT_FOUND);
        else
            $this->httpResponse->setCode(Response::S200_OK);
